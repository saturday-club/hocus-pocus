...
final class GrainRenderer {
    ...
    func start() {
        ...
        // Render once. Static grain like Monocle -- no regeneration needed.
        render()
    }

    /// Force a re-render (e.g. when intensity changes).
    func renderOnce() {
        render()
    }
    ...
    func updateSize(_ size: CGSize, scaleFactor: CGFloat) {
        // Full native resolution for crisp grain. Since we only render once, no perf concern.
        metalLayer.drawableSize = CGSize(
            width: max(1, size.width * scaleFactor),
            height: max(1, size.height * scaleFactor)
        )
        metalLayer.contentsScale = scaleFactor
        metalLayer.magnificationFilter = .nearest  // Sharp pixels, no blur
    }
